...
- 스타트업의 **모호한 기대수익 0**은 현금 운용수익 0으로 단독 확정하지 않고 `startup-finance`의 두 사업 해석 경계로 돌려보낸다.
- 변동성·표본·시나리오 확률 같은 **분포 입력이 없으면 `montecarlo`를 호출하지 않는다**. `financial-model`의 기존 계획과 무성장 대안을 나란히 계산해 결정론적 스트레스로 표시한다.
