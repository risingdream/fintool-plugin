...
- 스타트업 문맥만으로 기대수익의 대상을 확정하지 않는다. 지금 계획대로라는 표현만으로 현금 운용수익 0으로 단정하지 않는다. 사용자가 대상을 명시하지 않으면 질문하고, 바로 계산해야 하면 **현금 운용수익 0**과 **매출 성장 0**을 나란히 계산한다. **매출 0**도 숨은 기본값으로 쓰지 않는다.
