...
## Monte Carlo 가정 분리와 모델 적합성

스타트업 런웨이에서 다음 표현은 서로 다른 가정이다. 이름이 비슷하다는 이유로 바꿔 쓰지 않는다.

| 표현 | 뜻 |
|---|---|
| **기대수익 0** | 대상이 사업 기대수익인지 현금 운용수익인지 특정되지 않은 모호한 표현 |
| **매출 0** | 모든 사업 매출 유입을 제거한 무매출 스트레스 |
| **매출 성장 0** | 최신 기준 매출을 유지하는 무성장 가정. 매출 0이 아니다 |
| **현금 운용수익 0** | 현금 잔액의 운용수익률만 0. 사업 매출 계획은 유지 |

- **금지:** 스타트업 문맥만으로 기대수익의 대상을 확정하지 않는다. 모호한 `기대수익 0`을 **현금 운용수익 0 하나로 해석해 계산하지 않는다**. 지금 계획대로라는 표현만으로 현금 운용수익 0으로 단정하지 않는다. **바로 계산 요청은 모호성을 해소하지 않는다.**
- 사용자가 대상을 명시하지 않으면 질문한다. 바로 계산해야 하면 (A) 현금 운용수익 0 + 기존 사업 성장 유지, (B) 매출 성장 0 + 최신 월매출 유지의 **두 `financial-model` 결과**를 나란히 계산한다. **매출 0**은 숨은 기본값으로 쓰지 않는다.
- 두 결과는 한 번의 호출에서 base와 `scenarios:[{"name":"flat_growth","overrides":{"drivers.<id>.schedule.monthly_growth":0}}]`로 받는다. 시나리오 `name`은 영문 식별자여야 한다. 오버레이 evidence는 이름을 경로로 만들지 말고 `{"kind":"assumption","fields":["scenarios"],"note":"매출 성장 0 스트레스"}`처럼 플래그 전체에 붙인다.
- 바로 계산해야 하는 모호한 요청에서는 확률 입력을 되묻는 질문만 하고 종료하지 않는다. 먼저 위 `financial-model`을 실행해 성공 봉투에서 `base`와 `flat_growth`를 모두 확인한 뒤 실제 런웨이 결과를 답한다. 계산에 실패하면 결과를 추정하지 않고 실패를 알린다.
- 변동성·부트스트랩 표본·시나리오 확률처럼 경로를 가르는 **분포 입력이 없으면 `montecarlo`를 호출하지 않는다**. 두 결과를 확률이 아닌 **결정론적 스트레스**로 표시한다. `trials`와 `seed`를 임의로 보태 결정론적 입력을 확률 예측처럼 만들지 않는다.
- 멀티턴에서는 기존 `financial-model` 상태를 유지한다. 최신 Base 매출·성장률·원가율·채용·비용·조달을 보존하고, 이번 질문 때문에 별도 무매출 모델을 새로 만들지 않는다. `매출 0`은 사용자가 명시했을 때만 별도 스트레스로 추가한다.
- 사업의 월별 매출·원가·채용·조달 경로는 `financial-model`로 계산한다. 기존 계획을 결정론적 현금흐름으로 전개하는 것이 먼저다.
- `montecarlo` GBM은 현금 잔고에 자산 수익률 분포와 집계 현금흐름을 적용한다. 사업 매출 성장률·원가율·채용 불확실성을 직접 모델링하지 않는다. 이 값들의 확률을 주장하려면 각각의 분포 가정과 현금흐름 변환 방법을 먼저 명시해야 한다.
- `mean-return=0`, `volatility=0`이면 모든 경로가 같으므로 확률 예측이 아니다. 답변에 **결정론적 스트레스**라고 표시한다. 변동성이나 시나리오 확률을 명시해 경로가 갈리면 사용한 **분포 가정**과 그 대상이 사업인지 현금 운용인지 표시한다.
